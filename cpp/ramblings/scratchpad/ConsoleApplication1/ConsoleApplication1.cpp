...
bool readHeartbeat(std::string &host, std::string &port, std::string &command)
{
	try
	{
...
HANDLE exists()
{
	HANDLE h = CreateMutexA(0, FALSE, "Local\\zzz"); // try to create a named mutex
...
std:string theName = "C:/log/server.log";
...
int mainOld(int argc, char* argv[])
{
	...
	std::string in;
	std::string s = ssystem("dir c:\\src");
	...
	readHeartbeat(std::string(argv[1]), std::string(argv[2]), std::string(argv[3]));
	*/
